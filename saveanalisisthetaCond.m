function saveanalisisthetaCond (mainFolder,subfolders,finalfolder)
...
for i = 1:length(subfolders)
    ...
    dataFP_LFP = results.DLC.conductath.PETH.onset.allintFP;
...
filename_pre = fullfile(finalfolder, ['AnalysisResults_Pre_',nombre_raton,'LFP-Conducta.xlsx']);
writetable(T_pre, filename_pre);

filename_post = fullfile(finalfolder, ['AnalysisResults_Post_',nombre_raton,'LFP-Conducta.xlsx']);
writetable(T_post, filename_post);

% Combinar los resultados en una sola tabla (opcional)
T_raw_data = [T_pre(:, 1:2), T_post(:, 2)];
filename_raw_data = fullfile(finalfolder, ['AnalysisResults_RawData_',nombre_raton,'LFP-Conducta.xlsx']);
...
% Etiquetas y título
ylabel('Densidad');
title('Media y SEM (pre y post) - LFP//Conducta');
...
grid on;
saveas (gcf,fullfile(finalfolder,['Modulación ', nombre_raton,'-LFPyConducta.jpg']));
close all
...
% Etiquetas y título
ylabel('Potencia de theta');
title('Modulación de la theta - pre y post evento de movimiento');
xlim([0.5,length(subfolders)+0.5]);
xticks(1:length(subfolders));
xticklabels(rat_numbers); % Usar los números de ratón como etiquetas
grid on;
saveas(gcf,fullfile(finalfolder,['Potencia y modulación LFP con conducta_',nombre_raton,'.jpg']))
close all
end
