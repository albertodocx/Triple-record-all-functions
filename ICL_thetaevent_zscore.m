function Data = ICL_thetaevent_zscore(folderpath, varargin)
...
%         thetaevent_pseacheventmean(nn).(varname) = mean(mean(zscore(thetaevent_ps(nn).(varname)), 1), 2);
...
%         gammaevent_pseacheventmean(nn).(varname) = mean(mean(zscore(gammaevent_ps(nn).(varname)), 1), 2);
...
%         gammahighevent_pseacheventmean(nn).(varname) = mean(mean(zscore(gammahighevent_ps(nn).(varname)), 1), 2);
...
%         gammalowevent_pseacheventmean(nn).(varname) = mean(mean(zscore(gammalowevent_ps(nn).(varname)), 1), 2);
%         galow_eachevent(jj) = gammalowevent_pseacheventmean(nn).(varname);    
%     end

% Rangos para normalizar datos de las diferentes bandas de fq: 
% zscorefq = (freqs >= 1 & freqs <= 200).'; 
% pszscore = pow2db(thetach_profile(zscorefq, :));
% zscorefq = freqs(zscorefq);

%% CON EL RANGO CORRESPONDIENTE A CADA BANDA DE FREQ, PERO ZSCORE DE TODO EL REGISTRO (longitudinal)
% Aquí es la zscore de la banda de frq para todo el registro
% zscorefq = (freqs >= 1 & freqs <= 200).'; 
zscoretheta = zscore(mean(pow2db(thetach_profile(thetafq, :)),1));
zscoregamma = zscore(mean(pow2db(thetach_profile(gammafq, :)),1));
zscoregammahigh = zscore(mean(pow2db(thetach_profile(gammahigh, :)),1));
zscoregammalow = zscore(mean(pow2db(thetach_profile(gammalow, :)),1));

    for jj = 1:eventnr
        varname = sprintf('event%i', jj);
        % theta
        thetaevent_ps(nn).(varname) = zscoretheta(events(jj, 1):events(jj, 2));
        thetaevent_pseacheventmean(nn).(varname) = mean(thetaevent_ps(nn).(varname));
        th_eachevent(jj) = thetaevent_pseacheventmean(nn).(varname);
        % gamma (all)
        gammaevent_ps(nn).(varname) = zscoregamma(events(jj, 1):events(jj, 2)); 
        gammaevent_pseacheventmean(nn).(varname) = mean(gammaevent_ps(nn).(varname));
        ga_eachevent(jj) = gammaevent_pseacheventmean(nn).(varname);       
        % gamma(high)
        gammahighevent_ps(nn).(varname) = zscoregammahigh(events(jj, 1):events(jj, 2)); 
        gammahighevent_pseacheventmean(nn).(varname) = mean(gammahighevent_ps(nn).(varname));
        gahigh_eachevent(jj) = gammahighevent_pseacheventmean(nn).(varname);    
        % gamma(low)
        gammalowevent_ps(nn).(varname) = zscoregammalow(events(jj, 1):events(jj, 2)); 
        gammalowevent_pseacheventmean(nn).(varname) = mean(gammalowevent_ps(nn).(varname));
        galow_eachevent(jj) = gammalowevent_pseacheventmean(nn).(varname);    
    end
...
%% CON TODA LA BANDA DE FREQ DE 1 A 200: 
% aquí sería hacer la zscore de toda la matriz y después sacar los
% valores/media
% zscorefq = (freqs >= 1 & freqs <= 200).'; 
% pszscore = zscore(pow2db(thetach_profile(zscorefq, :)));
% zscorefq = freqs(zscorefq).';
% psthetaz = pszscore(zscorefq >= 5.5 & zscorefq <= 8.5, :);
% psgammaz = pszscore(zscorefq >= 30 & zscorefq <= 90, :);
% psgammahighz = pszscore(zscorefq > 60 & zscorefq <=90 , :);
% psgammalowz = pszscore(zscorefq > 30 & zscorefq <=60, :);
% 
% 
%     for jj = 1:eventnr
%         varname = sprintf('event%i', jj);
%         % theta
%         thetaevent_ps(nn).(varname) = psthetaz(:, events(jj, 1):events(jj, 2));
%         thetaevent_pseacheventmean(nn).(varname) = mean(mean(thetaevent_ps(nn).(varname),1),2);
%         th_eachevent(jj) = thetaevent_pseacheventmean(nn).(varname);
%         % gamma (all)
%         gammaevent_ps(nn).(varname) = psgammaz(:, events(jj, 1):events(jj, 2)); 
%         gammaevent_pseacheventmean(nn).(varname) = mean(mean(gammaevent_ps(nn).(varname),1),2);
%         ga_eachevent(jj) = gammaevent_pseacheventmean(nn).(varname);       
%         % gamma(high)
%         gammahighevent_ps(nn).(varname) = psgammahighz(:, events(jj, 1):events(jj, 2)); 
%         gammahighevent_pseacheventmean(nn).(varname) = mean(mean(gammahighevent_ps(nn).(varname),1),2);
%         gahigh_eachevent(jj) = gammahighevent_pseacheventmean(nn).(varname);    
%         % gamma(low)
%         gammalowevent_ps(nn).(varname) = psgammalowz(:, events(jj, 1):events(jj, 2)); 
%         gammalowevent_pseacheventmean(nn).(varname) = mean(mean(gammalowevent_ps(nn).(varname),1),2);
%         galow_eachevent(jj) = gammalowevent_pseacheventmean(nn).(varname);    
%     end
% 
% 
...
% 
   ...
   results.thetaeventzscore.pscompleteprofile = thetach_profile;
   results.thetaeventzscore.psdelta = thetach_psde;
   results.thetaeventzscore.pstheta = thetach_psth;
   results.thetaeventzscore.thetapstime = thetaevent_ps;
   results.thetaeventzscore.thetapsmean = thetaevent_pseacheventmean;
   results.thetaeventzscore.gammapstime = gammaevent_ps;
   results.thetaeventzscore.gammapsmean = gammaevent_pseacheventmean;
   results.thetaeventzscore.gammahighpsmean = gammahighevent_pseacheventmean;
   results.thetaeventzscore.gammalowpsmean = gammalowevent_pseacheventmean;
   results.thetaeventzscore.spthetafilt = sp_thetafilt;
   results.thetaeventzscore.slmthetafilt = thetafilt;
   
   results.thetaeventzscore.eventnr = eventnr;
   results.thetaeventzscore.evdurtot = evdurtot;
   results.thetaeventzscore.evdurmean = evdurmean;
   results.thetaeventzscore.idxs = events;

   results.thetaeventzscore.params.runthresh = runthresh; % threshold of zscore in std
   results.thetaeventzscore.params.LIAthresh = LIAthresh;
%        if thetachcalc == 1
%            results.thetaeventzscore.params.eventch = ripplech;
%        elseif thetachcalc == 2
           results.thetaeventzscore.params.eventch = thetach;
%        end
   ...
   %% 6. Save csv files: 
       ...
       savename = uigetdir();
       dataname = strcat(savename, '\tg_thetaevent_zscore.xlsx');
