function saveanalisisFPLFP(mainFolder, subfolders,finalfolder)
...
filename_pre = fullfile(finalfolder, ['AnalysisResults_Pre_',nombre_raton,'Fotometría-LFP.xlsx']);
writetable(T_pre, filename_pre);

filename_post = fullfile(finalfolder, ['AnalysisResults_Post_',nombre_raton,'Fotometría-LFP.xlsx']);
writetable(T_post, filename_post);

% Combinar los resultados en una sola tabla (opcional)
T_raw_data = [T_pre(:, 1:2), T_post(:, 2)];
filename_raw_data = fullfile(finalfolder, ['AnalysisResults_RawData_',nombre_raton,'Fotometría-LFP.xlsx']);
...
grid on;
saveas (gcf,fullfile(finalfolder,['Modulación ', nombre_raton,'-FotometríayLFP.jpg']));
close all
...
title('Modulación de la fotometría - pre y post theta event');
xlim([0.5,length(subfolders)+0.5]);
xticklabels(rat_numbers); % Usar los números de ratón como etiquetas
grid on;
saveas(gcf,fullfile(finalfolder,['Potencia y modulación_',nombre_raton,'.jpg']))
close all
end
