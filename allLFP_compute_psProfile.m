function [psProfile] = allLFP_compute_psProfile(basicData, config_channels, varargin)
...
% Edit:  Rut de la Vega 18-04-2023 (pendiente de ajustar config_channels)
...
delta_cf = [0, 3]; % Delta power: between 0 and 3Hz
...
hfo_cf = [100, 1e3]; % HFO power: between 100 and 1000Hz
...
fprintf('\n\n    --------------------\n    |  Power Spectrum  |\n    --------------------\n');
    ...
    %Quit channels in neuropixel
    if ismember(6, unique(shType))
        keepInd = 1:4:size(data, 2);
        shMap = shMap(keepInd);
        data = data(:, keepInd);
    else
    ...
    lenpS = length(pspectrum(double(data(1:round(tWin), 1)'),  fs, 'Reassign', true));
    pS = zeros(lenpS,nWin,nCh);
    % For each channel
    for ch = channels % Recorro todos los canales.
        for win = 1:nWin
            % Temporal data of the corresponding window
            dataWin = data(round(dtWin*(win-1)+1):round(dtWin*(win-1)+tWin),ch);
            % Storing results of power spectrum between 1 and 1000
            [pS(:,win,ch),freq] = pspectrum(double(dataWin'), fs, 'Reassign', true);
        end
    end
    ...
    for ch = channels
        ...
        for win = 1:nWin
            ...
            pdelta(win,ch) = 10*log10(sum(pS(freq>=delta_cf(1)&freq<=delta_cf(2), win, ch)))+60;
            % Theta power: between 4 and 12Hz
            ptheta(win,ch) = 10*log10(sum(pS(freq>=theta_cf(1)&freq<=theta_cf(2), win, ch)))+60;
            % Alpha power: between 9 and 16Hz
            palpha(win,ch) = 10*log10(sum(pS(freq>=alpha_cf(1)&freq<=alpha_cf(2), win, ch)))+60;
            % Gamma power: between 20 and 90Hz
            pgamma(win,ch)  = 10*log10(sum(pS(freq>=gamma_cf(1)&freq<=gamma_cf(2), win, ch)))+60;
            % HFO power: between 100 and 1000Hz
            try %y este catch, cuando sucede??
                phfo(win,ch) = 10*log10(sum(pS(freq>=hfo_cf(1)&freq<=hfo_cf(2), win, ch)))+60; 
            catch 
                phfo(win,ch) = nan; 
            end 
        end
    ...
    for iSh = 1:max(shMap)
        chPos = max(keepInd(shMap==iSh));
        plot( chPos+[.5 .5], [ax(3) ax(4)*1.02], '--', 'color', [.5 .5 .5], 'linewidth', 2 )
        text( chPos-4, ax(4)*0.99, sprintf('Shank #%d',iSh) )
        text( chPos-5, ax(4)*0.98, '<-- Skull  |   In -->', 'fontsize', 9 )
    end
    hold off
    ...
    % qu = input('Power spectrum of theta (y/n)? ','s');
    qu = 'n';
